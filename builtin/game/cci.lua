...
-- Keep native C++ draw_line_native if present, otherwise fallback
local draw_line_native = cci and cci.draw_line_native
...
cci.scale = 0.1         -- Coordinate multiplier to map e.g. (20, 20) to (2, 2) in formspec
cci.build_mode = true   -- Shows point markers and outline dots by default
...
-- Line drawing interpolation (Point Draw) using native C++ draw_line_native
local function draw_line(x1, y1, x2, y2, color, fs_elements)
	if draw_line_native then
		local parts = draw_line_native(x1, y1, x2, y2, color)
		for _, p in ipairs(parts) do
			table.insert(fs_elements, p)
		end
	else
...
local function ease_val(t, mode)
			...
			for _, obj in pairs(sess.objects) do
				for prop, anim in pairs(obj.animations) do
					...
					if t >= 1 then
						...
						local factor = ease_val(t, anim.easing)
						if anim.is_color then
