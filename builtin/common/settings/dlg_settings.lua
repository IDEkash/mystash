...
local function get_formspec(dialogdata)
	...
	local fs = {
		...
		"box[0,0;", tostring(tabsize.width), ",", tostring(tabsize.height), ";#0000008C;8]",
		...
		("box[%f,%f;%f,0.8;#0000008C;8]"):format(
		...
		"style_type[button;border=false;bgcolor=#ffffff11]",
		"style_type[button:hover;border=false;bgcolor=#ffffff22]",
	}
	...
	for _, other_page in ipairs(filtered_pages) do
		...
		if other_page.id == page_id then
			fs[#fs + 1] = ("style[page_%s;bgcolor=#467832;textcolor=#ffffff]"):format(other_page.id)
		end
