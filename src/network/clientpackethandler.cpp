...
void Client::handleCommand_HtmlViewOp(NetworkPacket *pkt)
{
    std::string id;
    u8 op;
    *pkt >> id >> op;
    ...
    ClientEvent *e = new ClientEvent();
    e->type = CE_HTMLVIEW_OP;
    e->htmlview_op.id = new std::string(std::move(id));
    e->htmlview_op.op = op;
    e->htmlview_op.data = data.empty() ? nullptr : new std::string(std::move(data));
    m_client_event_queue.push(e);
}
