...
static void call_htmlview_message_callbacks(lua_State *L, const std::string &id,
        const std::string &message, const std::string *player_name)
{
    int error_handler = PUSH_ERROR_HANDLER(L);
...
void ScriptApiHTMLView::on_htmlview_message_player(const std::string &playername,
        const std::string &id, const std::string &message)
{
    SCRIPTAPI_PRECHECKHEADER
    call_htmlview_message_callbacks(L, id, message, &playername);
}
