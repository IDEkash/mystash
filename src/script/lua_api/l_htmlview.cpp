...
int ModApiHTMLView::l_send_to_player(lua_State *L)
{
    NO_MAP_LOCK_REQUIRED;
    std::string playername = readParam<std::string>(L, 1);
    std::string id = readParam<std::string>(L, 2);
    std::string message = readParam<std::string>(L, 3);

    Server *server = getServer(L);
    RemotePlayer *player = server->getPlayer(playername);
    if (!player)
        return luaL_error(L, "Player not found: %s", playername.c_str());

    session_t peer_id = player->getPeerId();
    if (peer_id == PEER_ID_INEXISTENT)
        return luaL_error(L, "Player not connected: %s", playername.c_str());

    NetworkPacket pkt(TOSERVER_HTMLVIEW_MSG, 0, peer_id);
    pkt << id << message;
    server->Send(peer_id, &pkt);
    return 0;
}
...
void ModApiHTMLView::Initialize(lua_State *L, int top)
{
        ...
        registerFunction(L, "send_to_player", l_send_to_player, tbl);
