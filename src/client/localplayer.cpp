...
void LocalPlayer::applyControl(float dtime, Environment *env)
{
    ...
    if (superspeed || (is_climbing && fast_climb) ||
        ...
        // Apply sprint multiplier when moving above threshold on ground
        float speed_factor = control.movement_speed;
        if (touching_ground && !free_move && !in_liquid && !in_liquid_stable &&
                speed_factor > physics_override.sprint_threshold &&
                physics_override.sprint_multiplier > 1.0f) {
            speedH *= physics_override.sprint_multiplier;
        }
    }
