...
- `params`:
  ...
  - `fog_start` / `start`: number (`0..0.99`, fraction of view distance; pass a negative value to leave at engine default)
  - `fog_end` / `end`: number (`0..1`, fraction of view distance; pass a negative value to leave at engine default; clamped to ≥ `fog_start` when both are non-negative)
  - `blend_time`: number (seconds, transition duration; clamped to ≥ `0`)
  - `max_density` / `density`: number (`0..1`, opacity at max height; clamped)
  ...
  - `layers`: array of up to **4** extra fog layer tables (excess entries are silently dropped). Each layer supports: `color`, `max_density` (or `density`), `max_density_height`, `zero_density_height`, `uniform`, `direction`.
...
`core.get_fog(player) -> table | nil`
- Returns the active fog parameters table for a player, or `nil` if custom fog is not active.
...
`core.get_fog_boundary(player) -> table | nil`
- Returns the active fog boundary parameters table for a player, or `nil` if no boundary is set.
