...
- `params` fields:
  ...
  - Any other key-value pair will be treated as a world setting (e.g. `seed`, `mg_name`, `mg_flags`, `creative_mode`).
...
- Aliased as `dim.enter_world(world_path)`, `dim.load(world_path)`, and `dim.unload(world_path)`.
